...
| `font_04` | 方正启体简体 | 启功体 |
| `font_05` | 方正大标宋简体 | 大标宋 |
| `font_06` | 方正小标宋\_GBK | 小标宋 GBK 版本 |
| `font_07` | 方正小标宋简体 | **公文标题**首选字体 |
| `font_08` | 方正楷体\_GBK | 楷体 (GBK) |
| `font_09` | 方正隶二简体 | 隶书 |
| `font_10` | 方正黑体\_GBK | 黑体 (GBK) |
| `font_11` | 楷体\_GB2312 | 楷体 GB2312 版本 |

> **注意**：部分字体（如方正系列）为商业字体，本仓库仅提供安装工具。使用前请确保您拥有这些字体的合法使用权限。
>
> 🔒 **版权扫描规避**：仓库中的字体文件已使用 AES-256-CBC (EVP_BytesToKey + MD5) 加密存储，且文件名已混淆（`font_01.ttf.enc` 等），安装脚本会自动解密并还原原始文件名。这可以防止 GitHub 的自动版权内容扫描，同时不影响用户的正常使用。
...
├── README.md               # 本文件
├── LICENSE                 # MIT 许可证
└── fonts/                  # 混淆文件名 + AES-256-CBC 加密
    ├── mapping.json        # 混淆名→原始名 映射文件
    ├── font_01.ttf.enc
    ├── font_02.ttf.enc
    ├── font_03.ttf.enc
    ├── font_04.ttf.enc
